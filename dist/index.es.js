import { createElementBlock as p, openBlock as c, renderSlot as h, defineComponent as b, ref as v, computed as x, reactive as te, watch as L, Fragment as A, withDirectives as O, createCommentVNode as I, normalizeStyle as T, normalizeClass as S, vShow as N, createElementVNode as $, createVNode as W, createBlock as B, withCtx as V, toDisplayString as M, renderList as q, createTextVNode as R, createApp as H, h as U, provide as oe, unref as E, onUnmounted as G, onMounted as K, resolveDynamicComponent as de, markRaw as pe, inject as se, useAttrs as fe } from "vue";
import { gesture as J } from "@toife/gesture";
import { useRouter as ne, useRoute as me } from "vue-router";
const ve = (o) => o && (o.tagName == "INPUT" || o.tagName == "TEXTAREA" || o.isContentEditable), _e = () => {
  const o = document.activeElement;
  o && (o.tagName === "INPUT" || o.tagName === "TEXTAREA" || o.isContentEditable) && o.blur();
}, ye = () => {
  document.addEventListener("contextmenu", (o) => o.preventDefault()), document.addEventListener("selectstart", (o) => o.preventDefault()), document.addEventListener("pointerup", (o) => {
    !ve(o.target) && _e();
  });
}, _ = (o, s) => {
  const e = o.__vccOpts || o;
  for (const [t, n] of s)
    e[t] = n;
  return e;
}, ge = {}, he = { class: "t-app" };
function be(o, s) {
  return c(), p("div", he, [
    h(o.$slots, "default")
  ]);
}
const ke = /* @__PURE__ */ _(ge, [["render", be]]), $e = /* @__PURE__ */ b({
  __name: "t-present",
  props: {
    keepalive: { type: Boolean, default: !0 },
    visible: { type: Boolean, default: !1 },
    backdrop: { type: Boolean, default: !0 },
    placement: { default: "bottom" }
  },
  emits: ["dismiss"],
  setup(o, { expose: s, emit: e }) {
    const t = v(0), n = v(!1), a = v(), i = v(), r = o, u = e, m = x(() => n.value || r.keepalive), y = te({
      "--t-present-backdrop-opacity": "0.4",
      "--t-present-transition": "0.2s",
      "--t-present-content-transform": "0px",
      "--t-present-content-opacity": "1"
    }), l = (f) => {
      r.backdrop ? f?.backdropOpacity && (y["--t-present-backdrop-opacity"] = f.backdropOpacity) : y["--t-present-backdrop-opacity"] = "0", f?.transition && (y["--t-present-transition"] = f.transition, y["--t-present-transition"] = f.transition), f?.contentTransform && (y["--t-present-content-transform"] = f.contentTransform), f?.contentOpacity && (y["--t-present-content-opacity"] = f.contentOpacity);
    }, g = () => {
      l({
        contentTransform: "0px",
        transition: "0.2s",
        backdropOpacity: "0.4",
        contentOpacity: "1"
      });
    }, d = () => {
      let f = "0px", z = "1";
      r.placement == "bottom" || r.placement == "right" ? f = "100%" : r.placement == "top" || r.placement == "left" ? f = "-100%" : r.placement == "center" && (f = "0px", z = "0"), l({
        contentTransform: f,
        transition: "0.2s",
        contentOpacity: z,
        backdropOpacity: "0"
      });
    };
    L(() => r.visible, () => {
      r.visible ? (n.value = !0, t.value = Re.getNewIndex(), setTimeout(() => {
        g();
      }, 50)) : (d(), setTimeout(() => {
        n.value = !1;
      }, 200));
    }), s({
      render: l,
      open: g,
      close: d
    });
    const k = (f) => {
      f.preventDefault(), u("dismiss", "backdrop");
    };
    return d(), (f, z) => (c(), p(A, null, [
      m.value ? O((c(), p("div", {
        key: 0,
        class: S(["t-present", { [f.placement]: !0 }]),
        ref_key: "present",
        ref: i,
        style: T({ ...y, zIndex: t.value })
      }, [
        h(f.$slots, "default", {}, void 0, !0)
      ], 6)), [
        [N, n.value]
      ]) : I("", !0),
      m.value ? O((c(), p("div", {
        key: 1,
        class: "t-present-backdrop",
        onClick: k,
        ref_key: "backdrop",
        ref: a,
        style: T({ ...y, zIndex: t.value - 1 })
      }, null, 4)), [
        [N, n.value]
      ]) : I("", !0)
    ], 64));
  }
}), D = /* @__PURE__ */ _($e, [["__scopeId", "data-v-3d0c3227"]]), we = /* @__PURE__ */ b({
  __name: "t-loading-icon",
  props: {
    size: {
      type: String,
      default: "24px"
    },
    color: {
      type: String,
      default: "var(--t-color-text)"
    }
  },
  setup(o) {
    return (s, e) => (c(), p("span", {
      class: "t-loading-icon",
      style: T({
        width: o.size,
        height: o.size,
        borderTopColor: o.color
      }),
      role: "status",
      "aria-label": "Loading..."
    }, null, 4));
  }
}), X = /* @__PURE__ */ _(we, [["__scopeId", "data-v-7c463303"]]), Te = { class: "t-button-content" }, xe = {
  key: 0,
  class: "loader"
}, Se = /* @__PURE__ */ b({
  __name: "t-button",
  props: {
    color: { default: null },
    size: { default: "standard" },
    rounded: { type: Boolean, default: !1 },
    block: { type: Boolean, default: !1 },
    loading: { type: Boolean, default: !1 },
    loadingType: { default: "spinner" },
    variant: { default: "default" },
    activeBackground: { type: Boolean, default: !1 }
  },
  setup(o) {
    const s = o, e = x(() => {
      let t;
      return s.variant == "default" && (s.color ? ["warning", "info", "danger", "primary", "secondary", "success"].includes(s.color) ? t = {
        "--border": "none",
        "--color": "var(--t-color-status-" + s.color + "-text)",
        "--background": "var(--t-color-status-" + s.color + ")"
      } : t = {
        "--border": "none",
        "--color": "var(--t-color-text)",
        "--background": s.color
      } : t = {
        "--border": "none",
        "--color": "var(--t-color-text)",
        "--background": "var(--t-color-surface)"
      }), s.variant == "text" && (s.color ? ["warning", "info", "danger", "primary", "secondary", "success"].includes(s.color) ? t = {
        "--border": "none",
        "--color": "var(--t-color-status-" + s.color + ")",
        "--background": "transparent"
      } : t = {
        "--border": "none",
        "--color": s.color,
        "--background": "transparent"
      } : t = {
        "--border": "none",
        "--color": "var(--t-color-text)",
        "--background": "transparent"
      }), s.variant == "outline" && (s.color ? ["warning", "info", "danger", "primary", "secondary", "success"].includes(s.color) ? t = {
        "--border": "0.55px solid var(--t-color-status-" + s.color + ")",
        "--color": "var(--t-color-status-" + s.color + ")",
        "--background": "transparent"
      } : t = {
        "--border": "0.55px solid " + s.color,
        "--color": s.color,
        "--background": "transparent"
      } : t = {
        "--border": "0.55px solid var(--t-color-text)",
        "--color": "var(--t-color-text)",
        "--background": "transparent"
      }), t;
    });
    return (t, n) => (c(), p("button", {
      class: S(["t-button", { "active-background": s.activeBackground, rounded: s.rounded, block: s.block, ["size-" + s.size]: !0 }]),
      style: T(e.value)
    }, [
      O($("span", Te, [
        h(t.$slots, "default", {}, void 0, !0)
      ], 512), [
        [N, !s.loading]
      ]),
      s.loading ? (c(), p("span", xe, [
        W(X, {
          color: "var(--color)",
          type: s.loadingType
        }, null, 8, ["type"])
      ])) : I("", !0)
    ], 6));
  }
}), j = /* @__PURE__ */ _(Se, [["__scopeId", "data-v-185a9e49"]]), ze = {
  key: 0,
  class: "t-alert-header"
}, Ie = { class: "t-alert-content" }, Be = { class: "t-alert-footer" }, Ce = /* @__PURE__ */ b({
  __name: "t-alert",
  props: {
    title: {},
    message: {},
    actions: {},
    dismiss: {}
  },
  emits: ["dismiss"],
  setup(o, { expose: s, emit: e }) {
    const t = o, n = v(!1), a = e, i = v(), r = v(!1), u = () => {
      n.value = !0;
    }, m = (l) => {
      n.value = !1, l.handler && l.handler(), a("dismiss", "choose", l?.data);
    }, y = (l) => {
      t.dismiss && t.dismiss.includes(l) ? (n.value = !1, a("dismiss", l)) : l == "backdrop" && (r.value = !0, setTimeout(() => {
        r.value = !1;
      }, 300));
    };
    return s({
      open: u
    }), (l, g) => (c(), B(D, {
      placement: "center",
      backdrop: !0,
      keepalive: !1,
      visible: n.value,
      onDismiss: y
    }, {
      default: V(() => [
        $("div", {
          class: S(["t-alert", { pop: r.value }]),
          ref_key: "container",
          ref: i
        }, [
          h(l.$slots, "header", {}, () => [
            t.title ? (c(), p("div", ze, M(t.title), 1)) : I("", !0)
          ], !0),
          h(l.$slots, "content", {}, () => [
            $("div", Ie, M(t.message), 1)
          ], !0),
          h(l.$slots, "footer", {}, () => [
            $("div", Be, [
              (c(!0), p(A, null, q(t.actions, (d) => (c(), B(j, {
                color: d.color,
                variant: d.variant,
                onClick: (k) => m(d)
              }, {
                default: V(() => [
                  R(M(d.text), 1)
                ]),
                _: 2
              }, 1032, ["color", "variant", "onClick"]))), 256))
            ])
          ], !0)
        ], 2)
      ]),
      _: 3
    }, 8, ["visible"]));
  }
}), re = /* @__PURE__ */ _(Ce, [["__scopeId", "data-v-05d0c578"]]), co = (o = {}) => ({
  open() {
    return new Promise((s) => {
      let e = document.body.querySelector(".t-app");
      if (!e) return;
      const t = document.createElement("div"), n = v();
      e.appendChild(t);
      const a = H({
        render() {
          return U(re, {
            ...o,
            ref: n,
            onDismiss: (i, r) => {
              setTimeout(() => {
                a.unmount(), t.remove();
              }, 300), s({ type: i, data: r });
            }
          });
        }
      });
      a.mount(t), setTimeout(() => {
        n.value?.open?.();
      }, 50);
    });
  }
}), Ee = /* @__PURE__ */ b({
  __name: "t-action",
  props: {
    actions: {},
    dismiss: {}
  },
  emits: ["dismiss"],
  setup(o, { expose: s, emit: e }) {
    const t = o, n = v(!1), a = e, i = v(), r = v(!1), u = () => {
      n.value = !0;
    }, m = (l) => {
      n.value = !1, l.handler && l.handler(), a("dismiss", "choose", l?.data);
    }, y = (l) => {
      t.dismiss && t.dismiss.includes(l) ? (n.value = !1, a("dismiss", l)) : l == "backdrop" && (r.value = !0, setTimeout(() => {
        r.value = !1;
      }, 300));
    };
    return s({
      open: u
    }), (l, g) => (c(), B(D, {
      placement: "bottom",
      backdrop: !0,
      keepalive: !1,
      visible: n.value,
      onDismiss: y
    }, {
      default: V(() => [
        $("div", {
          class: S(["t-action", { pop: r.value }]),
          ref_key: "container",
          ref: i
        }, [
          (c(!0), p(A, null, q(t.actions, (d) => (c(), p("div", null, [
            (c(!0), p(A, null, q(d, (k) => (c(), B(j, {
              color: k.color,
              size: k.size,
              variant: k.variant,
              onClick: (f) => m(k),
              block: ""
            }, {
              default: V(() => [
                R(M(k.text), 1)
              ]),
              _: 2
            }, 1032, ["color", "size", "variant", "onClick"]))), 256))
          ]))), 256))
        ], 2)
      ]),
      _: 1
    }, 8, ["visible"]));
  }
}), Ve = /* @__PURE__ */ _(Ee, [["__scopeId", "data-v-c6bccffb"]]), io = (o = {}) => ({
  open() {
    return new Promise((s) => {
      let e = document.body.querySelector(".t-app");
      if (!e) return;
      const t = document.createElement("div"), n = v();
      e.appendChild(t);
      const a = H({
        render() {
          return U(Ve, {
            ...o,
            ref: n,
            onDismiss: (i, r) => {
              setTimeout(() => {
                a.unmount(), t.remove();
              }, 300), s({ type: i, data: r });
            }
          });
        }
      });
      a.mount(t), setTimeout(() => {
        n.value?.open?.();
      }, 50);
    });
  }
}), Me = { class: "t-loading" }, Pe = /* @__PURE__ */ b({
  __name: "t-loading",
  props: {
    type: { default: "spinner" }
  },
  emits: ["dismiss"],
  setup(o, { expose: s, emit: e }) {
    const t = o, n = v(!1);
    return s({
      open: () => {
        n.value = !0;
      },
      close: () => {
        n.value = !1;
      }
    }), (r, u) => (c(), B(D, {
      placement: "center",
      backdrop: !0,
      keepalive: !1,
      visible: n.value
    }, {
      default: V(() => [
        $("div", Me, [
          h(r.$slots, "default", {}, () => [
            W(X, {
              type: t.type
            }, null, 8, ["type"])
          ], !0)
        ])
      ]),
      _: 3
    }, 8, ["visible"]));
  }
}), Ae = /* @__PURE__ */ _(Pe, [["__scopeId", "data-v-8f84c14e"]]), uo = (o = {}) => {
  const s = v(), e = document.body.querySelector(".t-app");
  let t = null, n = null;
  return {
    open() {
      return new Promise((a) => {
        e && (n = document.createElement("div"), e.appendChild(n), t = H({
          render() {
            return U(Ae, {
              ...o,
              ref: s
            });
          }
        }), t.mount(n), setTimeout(() => {
          s.value?.open?.(), a(!0);
        }, 50));
      });
    },
    close() {
      return new Promise((a) => {
        s.value?.close?.(), setTimeout(() => {
          t?.unmount?.(), n?.remove?.(), a(!0);
        }, 300);
      });
    }
  };
}, Y = v(1e3), Re = {
  getNewIndex() {
    return Y.value += 2, Y.value;
  }
}, C = te([]), Q = v(!0), De = (o) => {
  C.push(o);
}, Oe = (o) => {
  o || C.pop();
}, Ne = () => {
  C.splice(0, C.length);
}, qe = () => {
  Q.value = !1;
}, Le = () => {
  Q.value = !0;
}, w = {
  screens: C,
  addScreen: De,
  removeScreen: Oe,
  removeAllScreen: Ne,
  lockSwipe: qe,
  unlockSwipe: Le,
  swipeable: Q,
  hasPrevious: x(() => C.length > 1),
  currentScreen: x(() => C[C.length - 1]),
  lastScreen: x(() => {
    if (!(C.length < 2))
      return C[C.length - 2];
  })
}, We = /* @__PURE__ */ b({
  __name: "t-toast",
  props: {
    message: {},
    space: { default: "0px" },
    placement: { default: "bottom" },
    duration: { default: 2e3 },
    color: { default: null },
    variant: { default: "default" }
  },
  emits: ["dismiss"],
  setup(o, { expose: s, emit: e }) {
    const t = o, n = e, a = v(!1), i = v(!1), r = v(!1), u = () => {
      i.value = !0, r.value = !1, setTimeout(() => {
        a.value = !0;
      }, 10), setTimeout(() => {
        m();
      }, t.duration + 10);
    }, m = () => {
      r.value = !0, setTimeout(() => {
        i.value = !1, a.value = !1, n("dismiss");
      }, 300);
    }, y = x(() => {
      let l;
      return t.variant == "default" && (t.color ? ["warning", "info", "danger", "primary", "secondary", "success"].includes(t.color) ? l = {
        "--border": "none",
        "--color": "var(--t-color-status-" + t.color + "-text)",
        "--background": "var(--t-color-status-" + t.color + ")"
      } : l = {
        "--border": "none",
        "--color": "var(--t-color-text)",
        "--background": t.color
      } : l = {
        "--border": "none",
        "--color": "var(--t-color-text)",
        "--background": "var(--t-color-surface)"
      }), t.variant == "text" && (t.color ? ["warning", "info", "danger", "primary", "secondary", "success"].includes(t.color) ? l = {
        "--border": "none",
        "--color": "var(--t-color-status-" + t.color + ")",
        "--background": "transparent"
      } : l = {
        "--border": "none",
        "--color": t.color,
        "--background": "transparent"
      } : l = {
        "--border": "none",
        "--color": "var(--t-color-text)",
        "--background": "transparent"
      }), t.variant == "outline" && (t.color ? ["warning", "info", "danger", "primary", "secondary", "success"].includes(t.color) ? l = {
        "--border": "0.55px solid var(--t-color-status-" + t.color + ")",
        "--color": "var(--t-color-status-" + t.color + ")",
        "--background": "transparent"
      } : l = {
        "--border": "0.55px solid " + t.color,
        "--color": t.color,
        "--background": "transparent"
      } : l = {
        "--border": "0.55px solid var(--t-color-text)",
        "--color": "var(--t-color-text)",
        "--background": "transparent"
      }), l;
    });
    return s({
      open: u
    }), (l, g) => i.value ? (c(), p("div", {
      key: 0,
      class: S(["t-toast", { [t.placement]: !0, open: a.value, closing: r.value }]),
      style: T({ "--space": t.space, ...y.value })
    }, M(t.message), 7)) : I("", !0);
  }
}), ae = /* @__PURE__ */ _(We, [["__scopeId", "data-v-7754026e"]]), po = (o = {}) => ({
  open() {
    let s = document.body.querySelector(".t-app");
    if (!s) return;
    const e = document.createElement("div"), t = v();
    s.appendChild(e);
    const n = H({
      render() {
        return U(ae, {
          ...o,
          ref: t,
          onDismiss: () => {
            setTimeout(() => {
              n.unmount(), e.remove();
            }, 300);
          }
        });
      }
    });
    n.mount(e), setTimeout(() => {
      t.value?.open?.();
    }, 50);
  }
}), He = v(0);
function le() {
  return {
    height: He
  };
}
const Ue = /* @__PURE__ */ b({
  __name: "t-cable",
  props: {
    keyboard: { type: Boolean, default: !0 },
    placement: { default: "bottom" }
  },
  setup(o) {
    const s = o, { height: e } = le();
    return oe("cableState", {
      placement: s.placement
    }), (t, n) => (c(), p("div", {
      class: S(["t-cable", { keyboard: s.keyboard && E(e) > 0, [s.placement]: !0 }]),
      style: T({ "--t-keyboard-height": E(e) + "px", "--t-keyboard-transition": E(e) > 0 ? "0.3s" : "0.1s" })
    }, [
      h(t.$slots, "default", {}, void 0, !0)
    ], 6));
  }
}), Xe = /* @__PURE__ */ _(Ue, [["__scopeId", "data-v-5234450f"]]), je = /* @__PURE__ */ b({
  __name: "t-card",
  props: {
    color: { default: null }
  },
  setup(o) {
    const s = o, e = x(() => {
      let t;
      return s.color ? ["warning", "info", "danger", "primary", "secondary", "success"].includes(s.color) ? t = {
        "--background": "var(--t-color-status-" + s.color + ")"
      } : t = {
        "--background": s.color
      } : t = {
        "--background": "var(--t-color-surface)"
      }, t;
    });
    return (t, n) => (c(), p("div", {
      class: "t-card",
      style: T(e.value)
    }, [
      h(t.$slots, "default")
    ], 4));
  }
}), Fe = {}, Ge = { class: "t-content" };
function Ke(o, s) {
  return c(), p("div", Ge, [
    h(o.$slots, "default", {}, void 0, !0)
  ]);
}
const ce = /* @__PURE__ */ _(Fe, [["render", Ke], ["__scopeId", "data-v-af750e89"]]), ee = 80, Je = /* @__PURE__ */ b({
  __name: "t-refresher",
  emits: ["refresh"],
  setup(o, { emit: s }) {
    const e = s, t = v(0), n = v(!1), a = v();
    let i, r = !1;
    const u = () => {
      n.value = !1, t.value = 0, i && i.cancel(), setTimeout(() => {
        r = !1;
      }, 2e3);
    }, m = () => {
      r = !0, n.value = !0, t.value = ee, e("refresh", u);
    };
    return L(() => a.value, () => {
      i && i.destroy();
      let y = a.value.closest(".t-content");
      y && (i = J(y, {
        isMoving: !1,
        options: {
          minDist: 60
        },
        down() {
          this.isMoving = !1;
        },
        move({ deltaY: l, initialDirection: g }) {
          n.value || r || g != "down" || (l >= 120 ? (this.isMoving = !1, m()) : (l > 10 || this.isMoving) && (this.isMoving = !0, t.value = l > 0 ? l : 0));
        },
        up({ deltaY: l, initialDirection: g }) {
          this.isMoving = !1, !(n.value || r) && (l > ee && g == "down" ? m() : t.value = 0);
        },
        cancel() {
          this.isMoving = !1, !(n.value || r) && (n.value = !1, t.value = 0);
        }
      }, {
        passive: !1
      }));
    }), G(() => {
      i && i.destroy();
    }), (y, l) => O((c(), p("div", {
      class: "t-refresher",
      ref_key: "container",
      ref: a,
      style: T({ height: `${t.value}px` })
    }, [
      $("img", {
        class: S({ spiner: n.value }),
        src: "https://static.toikit.com/toikit/home_2.png?v=3",
        alt: "Loading...",
        style: { width: "22px", height: "22px" }
      }, null, 2)
    ], 4)), [
      [N, t.value > 0]
    ]);
  }
}), Qe = /* @__PURE__ */ _(Je, [["__scopeId", "data-v-c6364c14"]]), Ze = {}, Ye = { class: "t-screen" };
function et(o, s) {
  return c(), p("div", Ye, [
    h(o.$slots, "default", {}, void 0, !0)
  ]);
}
const ie = /* @__PURE__ */ _(Ze, [["render", et], ["__scopeId", "data-v-c2a3436c"]]), tt = /* @__PURE__ */ b({
  __name: "t-swipe-screen",
  emits: ["change"],
  setup(o, { emit: s }) {
    let e = {};
    const t = ne(), n = me(), a = t.getRoutes(), i = v(!1), r = s;
    for (let d of a)
      e[d.name] = d.components;
    const u = (d) => {
      d && w.addScreen({
        name: d,
        target: null,
        component: pe(e[d] || null)
      });
    }, m = (d, k) => {
      !k || w.screens[d].target || (w.screens[d].target = k.$el, d > 0 && (i.value = !0, k.$el.style.transform = "translateX(var(--t-app-width))", k.$el.transitionOrigin = "center", document.documentElement.style.setProperty("--t-screen-backdrop-duration", "0s"), document.documentElement.style.setProperty("--t-swipe-backdrop-opacity", "0"), setTimeout(() => {
        k.$el.style.transition = "transform 0.35s ease", k.$el.style.transform = "translateX(0px)", document.documentElement.style.setProperty("--t-screen-backdrop-duration", "0.35s"), document.documentElement.style.setProperty("--t-swipe-backdrop-opacity", "0.5"), w.screens.length > 1 && (w.lastScreen.value.target.style.transitionOrigin = "left center", w.lastScreen.value.target.style.transition = "transform 0.35s ease", w.lastScreen.value.target.style.transform = "translateX(calc(var(--t-app-width) / 100 * 30 * -1)) scale(0.5) perspective(var(--t-app-width)) rotateY(30deg)"), setTimeout(() => {
          r("change"), i.value = !1;
        }, 400);
      }, 100)));
    }, y = () => {
      w.currentScreen.value.target.style.transition = "transform 0.35s ease", w.currentScreen.value.target.style.transform = "translateX(var(--t-app-width)) scale(1)", w.lastScreen.value.target.style.transition = "transform 0.35s ease", w.lastScreen.value.target.style.transform = "translateX(0px) scale(1) perspective(var(--t-app-width)) rotateY(0deg)", document.documentElement.style.setProperty("--t-screen-backdrop-duration", "0.35s"), document.documentElement.style.setProperty("--t-swipe-backdrop-opacity", "0"), i.value = !0, setTimeout(() => {
        w.removeScreen(), r("change"), i.value = !1;
      }, 400);
    };
    u(n.name), L(() => n.name, (d, k) => {
      d != w.currentScreen.value.name && (w.lastScreen.value?.name == d ? y() : u(d));
    });
    const l = () => {
      i.value = !0;
      const d = w.currentScreen.value.target, k = w.lastScreen.value.target;
      d.style.transition = "transform 0.35s ease", d.style.transform = "translateX(0px)", k.style.transition = "transform 0.35s ease", k.style.transform = "translateX(calc(var(--t-app-width) / 100 * 30 * -1)) scale(0.5) perspective(var(--t-app-width)) rotateY(30deg)", document.documentElement.style.setProperty("--t-screen-backdrop-duration", "0.35s"), document.documentElement.style.setProperty("--t-swipe-backdrop-opacity", "0.5"), setTimeout(() => {
        i.value = !1;
      }, 400);
    };
    let g;
    return K(() => {
      g = J(document.body, {
        isMoving: !1,
        beforeEvent(d) {
          return !(i.value || !w.swipeable.value || w.screens.length < 2);
        },
        fast({ initialDirection: d }) {
          d == "right" && t.back();
        },
        down() {
          this.isMoving = !1;
        },
        move({ deltaX: d, initialDirection: k }) {
          if (k != "right") return;
          const f = window.innerWidth, z = d / f * 100, P = w.currentScreen.value.target, Z = w.lastScreen?.value?.target;
          (d > 15 && d <= f || this.isMoving) && (d = d > 0 ? d : 0, this.isMoving = !0, P.style.transition = "transform 0s ease", P.style.transform = `translateX(${d}px)`, Z.style.transition = "transform 0s ease", document.documentElement.style.setProperty("--t-screen-backdrop-duration", "0s"), Z.style.transform = `translateX(calc((var(--t-app-width) / 100 * 30 * -1) + ((var(--t-app-width) / 100 * 30) / 100 * ${z}))) scale(${0.5 + 0.5 / 100 * z}) perspective(var(--t-app-width)) rotateY(${30 - 30 / 100 * z}deg)`, document.documentElement.style.setProperty("--t-swipe-backdrop-opacity", `${0.5 - 0.5 / 100 * z}`));
        },
        up({ deltaX: d, initialDirection: k }) {
          this.isMoving = !1, k != "right" && l();
          const f = window.innerWidth;
          d / f * 100 >= 50 ? t.back() : l();
        },
        cancel() {
          this.isMoving = !1, l();
        }
      });
    }), G(() => {
      g && g.destroy(), w.removeAllScreen();
    }), (d, k) => (c(), p(A, null, [
      (c(!0), p(A, null, q(E(w).screens, (f, z) => (c(), B(ie, {
        ref_for: !0,
        ref: (P) => m(z, P),
        style: T({ zIndex: z + (z == E(w).screens.length - 1 ? 2 : 1) }),
        key: z
      }, {
        default: V(() => [
          (c(), B(de(f.component.default)))
        ]),
        _: 2
      }, 1032, ["style"]))), 128)),
      $("div", {
        class: "t-swipe-backdrop",
        style: T({ zIndex: E(w).screens.length })
      }, null, 4)
    ], 64));
  }
}), ot = /* @__PURE__ */ _(tt, [["__scopeId", "data-v-abea77ec"]]), st = /* @__PURE__ */ b({
  __name: "t-toolbar",
  props: {
    placement: { default: null },
    safe: { type: Boolean, default: !0 },
    size: { default: "50px" }
  },
  setup(o) {
    const s = o, e = se("cableState"), t = x(() => s?.placement || e?.placement);
    return (n, a) => (c(), p("div", {
      class: S(["t-toolbar", { [t.value]: !0, safe: s.safe }]),
      style: T({ "--t-size-toolbar": s.size })
    }, [
      $("div", null, [
        h(n.$slots, "default", {}, void 0, !0)
      ])
    ], 6));
  }
}), nt = /* @__PURE__ */ _(st, [["__scopeId", "data-v-1ee9866d"]]), rt = /* @__PURE__ */ b({
  __name: "t-back-button",
  props: {
    to: {}
  },
  emits: ["back"],
  setup(o, { emit: s }) {
    const e = s, t = ne(), n = o, a = fe(), i = (r) => {
      if (a.onBack) {
        e("back");
        return;
      }
      w.hasPrevious.value ? t?.back?.() : n.to && t?.push?.(n.to);
    };
    return (r, u) => r.to || E(w).hasPrevious ? (c(), p("button", {
      key: 0,
      class: "t-back-button",
      onClick: i
    }, [
      h(r.$slots, "default", {}, () => [
        u[0] || (u[0] = $("i", { class: "ri-arrow-left-s-line" }, null, -1))
      ], !0)
    ])) : I("", !0);
  }
}), at = /* @__PURE__ */ _(rt, [["__scopeId", "data-v-839498f3"]]), lt = /* @__PURE__ */ b({
  __name: "t-text",
  props: {
    color: { default: "var(--t-color-text)" },
    size: { default: "standard" }
  },
  setup(o) {
    const s = o, e = x(() => {
      let t, n = s.color;
      return s.size == "standard" ? t = "var(--t-fs-10)" : s.size == "small" ? t = "var(--t-fs-08)" : s.size == "large" ? t = "var(--t-fs-12)" : t = s.size, ["warning", "info", "danger", "primary", "secondary", "success"].includes(s.color) && (n = `var(--t-color-status-${s.color})`), {
        "--color": n,
        "--font-size": t
      };
    });
    return (t, n) => (c(), p("span", {
      class: "t-text",
      style: T(e.value)
    }, [
      h(t.$slots, "default", {}, void 0, !0)
    ], 4));
  }
}), F = /* @__PURE__ */ _(lt, [["__scopeId", "data-v-f0f94f05"]]), ct = /* @__PURE__ */ b({
  __name: "t-pull-signal",
  props: {
    placement: { default: "bottom" }
  },
  setup(o) {
    return (s, e) => (c(), p("div", {
      class: S(["t-pull-bar", { [s.placement]: !0 }])
    }, null, 2));
  }
}), ue = /* @__PURE__ */ _(ct, [["__scopeId", "data-v-daa38c85"]]), it = /* @__PURE__ */ b({
  __name: "t-sheet",
  props: {
    background: { default: "var(--t-color-surface)" },
    visible: { type: Boolean, default: !1 },
    gesture: { type: Boolean, default: !0 },
    fullscreen: { type: Boolean, default: !1 },
    placement: { default: "bottom" },
    keepalive: { type: Boolean, default: !0 },
    backdrop: { type: Boolean, default: !0 },
    rounded: { type: Boolean, default: !0 },
    radius: { type: Boolean, default: !0 },
    closeButton: { type: Boolean, default: !0 }
  },
  emits: ["dismiss"],
  setup(o, { emit: s }) {
    const e = o, t = s, n = v(), a = v(), i = v(!1);
    let r = null;
    const u = x(() => {
      if (e.placement == "bottom") return "down";
      if (e.placement == "top") return "up";
      if (e.placement == "left") return "left";
      if (e.placement == "right") return "right";
    }), m = (l) => {
      t("dismiss", l);
    }, y = () => {
      i.value = !0, setTimeout(() => {
        i.value = !1;
      }, 300);
    };
    return L(() => n.value, (l) => {
      l && (r = J(n.value, {
        isMoving: !1,
        options: {
          minDist: 30
        },
        beforeEvent(g) {
          return !(i.value || !e.gesture || e.placement == "center");
        },
        down() {
          this.isMoving = !1;
        },
        fast({ initialDirection: g }) {
          y(), g == u.value ? t("dismiss", "gesture") : a.value.open();
        },
        move({ deltaY: g, deltaX: d, initialDirection: k }) {
          if (k != u.value) return;
          let f = 0;
          e.placement == "bottom" || e.placement == "top" ? f = g : f = d, e.placement == "bottom" && (f = g > 0 ? g : 0), e.placement == "top" && (f = g < 0 ? g : 0), e.placement == "left" && (f = d < 0 ? g : 0), e.placement == "right" && (f = d > 0 ? g : 0), (e.placement == "bottom" && (f >= 10 || this.isMoving) || e.placement == "top" && (f <= -10 || this.isMoving) || e.placement == "left" && (f >= 10 || this.isMoving) || e.placement == "right" && (f <= -10 || this.isMoving)) && (this.isMoving = !0, a.value.render({
            contentTransform: f + "px",
            transition: "0s"
          }));
        },
        up({ deltaY: g, deltaX: d, initialDirection: k }) {
          if (this.isMoving = !1, y(), k != u.value) {
            a.value.open();
            return;
          }
          let f, z, P;
          e.placement == "bottom" || e.placement == "top" ? (f = n.value.offsetHeight, P = g) : (f = n.value.offsetWidth, P = d), z = P / f * 100, z > 50 ? t("dismiss", "gesture") : a.value.open();
        },
        cancel() {
          this.isMoving = !1, y(), a.value.open();
        }
      }));
    }), G(() => {
      r && r.destroy();
    }), (l, g) => (c(), B(D, {
      ref_key: "present",
      ref: a,
      placement: e.placement,
      backdrop: l.backdrop,
      visible: e.visible,
      keepalive: e.keepalive,
      onDismiss: m
    }, {
      default: V(() => [
        e.gesture && e.placement != "center" ? (c(), B(ue, {
          key: 0,
          placement: e.placement
        }, null, 8, ["placement"])) : I("", !0),
        e.closeButton && e.placement == "center" ? (c(), p("button", {
          key: 1,
          class: "t-sheet-close",
          onClick: g[0] || (g[0] = (d) => m("close-button"))
        }, [
          h(l.$slots, "close-icon", {}, () => [
            g[1] || (g[1] = $("i", { class: "ri-close-large-line" }, null, -1))
          ], !0)
        ])) : I("", !0),
        $("div", {
          class: S(["t-sheet", { fullscreen: e.fullscreen, [e.placement]: !0, rounded: l.rounded, radius: e.radius }]),
          style: T({ "--background": e.background }),
          ref_key: "sheet",
          ref: n
        }, [
          h(l.$slots, "default", {}, void 0, !0)
        ], 6)
      ]),
      _: 3
    }, 8, ["placement", "backdrop", "visible", "keepalive"]));
  }
}), ut = /* @__PURE__ */ _(it, [["__scopeId", "data-v-0ff00a13"]]), dt = { class: "t-input-label" }, pt = { class: "t-input-content" }, ft = ["type", "placeholder", "value"], mt = /* @__PURE__ */ b({
  __name: "t-input",
  props: {
    size: { default: "standard" },
    rounded: { type: Boolean, default: !1 },
    label: { default: "" },
    placeholder: { default: "" },
    type: { default: "text" },
    help: {},
    error: {},
    modelValue: { default: "" },
    variant: { default: "default" }
  },
  emits: ["update:modelValue"],
  setup(o, { emit: s }) {
    const e = o, t = s, n = (a) => {
      t("update:modelValue", a.target.value);
    };
    return (a, i) => (c(), p("div", {
      class: S(["t-input", { rounded: e.rounded, ["size-" + e.size]: !0, ["variant-" + e.variant]: !0 }])
    }, [
      $("label", null, [
        $("span", dt, M(a.label), 1),
        $("div", pt, [
          h(a.$slots, "start", {}, void 0, !0),
          $("input", {
            type: a.type,
            placeholder: a.placeholder,
            value: a.modelValue,
            onInput: n
          }, null, 40, ft),
          h(a.$slots, "end", {}, void 0, !0)
        ])
      ]),
      e.error ? (c(), B(F, {
        key: 0,
        color: "danger"
      }, {
        default: V(() => [
          R(M(e.error), 1)
        ]),
        _: 1
      })) : e.help ? (c(), B(F, {
        key: 1,
        color: "secondary"
      }, {
        default: V(() => [
          R(M(e.help), 1)
        ]),
        _: 1
      })) : I("", !0)
    ], 2));
  }
}), vt = /* @__PURE__ */ _(mt, [["__scopeId", "data-v-1a9425cf"]]), _t = {};
function yt(o, s) {
  return c(), p("textarea");
}
const gt = /* @__PURE__ */ _(_t, [["render", yt]]), ht = {};
function bt(o, s) {
  return c(), p("div");
}
const kt = /* @__PURE__ */ _(ht, [["render", bt]]), $t = {}, wt = { class: "t-grid" };
function Tt(o, s) {
  return c(), p("div", wt);
}
const xt = /* @__PURE__ */ _($t, [["render", Tt]]), St = {}, zt = { class: "t-grid-item" };
function It(o, s) {
  return c(), p("div", zt);
}
const Bt = /* @__PURE__ */ _(St, [["render", It]]), Ct = /* @__PURE__ */ b({
  __name: "t-divider",
  props: {
    direction: { default: "horizontal" }
  },
  setup(o) {
    const s = o;
    return (e, t) => (c(), p("div", {
      class: S(["t-divider", { [s.direction]: !0 }])
    }, null, 2));
  }
}), Et = /* @__PURE__ */ _(Ct, [["__scopeId", "data-v-47ee8991"]]), Vt = /* @__PURE__ */ b({
  __name: "t-toggle-password",
  props: {
    color: { default: "warning" }
  },
  emits: ["change"],
  setup(o, { emit: s }) {
    const e = o, t = s, n = v(!1), a = v(), i = x(() => {
      let m = e.color;
      return ["warning", "info", "danger", "primary", "secondary", "success"].includes(e.color) && (m = `var(--t-color-status-${e.color})`), {
        "--color": m
      };
    }), r = () => a.value?.parentElement?.querySelector?.("input");
    K(() => {
      let m = r();
      m && (n.value = m.type != "password", t("change", n.value));
    });
    const u = () => {
      let m = r();
      m && (n.value = !n.value, n.value ? m.type = "text" : m.type = "password", t("change", n.value));
    };
    return (m, y) => (c(), p("button", {
      ref_key: "toggle",
      ref: a,
      class: "t-toggle-password",
      style: T(i.value),
      onClick: u
    }, [
      n.value ? h(m.$slots, "on", { key: 0 }, () => [
        y[0] || (y[0] = $("i", { class: "ri-eye-line" }, null, -1))
      ], !0) : I("", !0),
      n.value ? I("", !0) : h(m.$slots, "off", { key: 1 }, () => [
        y[1] || (y[1] = $("i", { class: "ri-eye-off-line" }, null, -1))
      ], !0)
    ], 4));
  }
}), Mt = /* @__PURE__ */ _(Vt, [["__scopeId", "data-v-3268e9e3"]]), Pt = /* @__PURE__ */ b({
  __name: "t-avatar",
  props: {
    square: { type: Boolean, default: !1 },
    size: { default: "standard" }
  },
  setup(o) {
    const s = o, e = x(() => s.size == "small" ? "24px" : s.size == "standard" ? "40px" : s.size == "large" ? "60px" : s.size);
    return (t, n) => (c(), p("div", {
      class: S(["t-avatar", { square: t.square }]),
      style: T({ "--size": e.value })
    }, [
      h(t.$slots, "default", {}, void 0, !0)
    ], 6));
  }
}), At = /* @__PURE__ */ _(Pt, [["__scopeId", "data-v-24384c2f"]]), Rt = { class: "t-collapse-title" }, Dt = {
  key: 0,
  class: "icon"
}, Ot = {
  key: 1,
  class: "icon"
}, Nt = { class: "t-collapse-content" }, qt = /* @__PURE__ */ b({
  __name: "t-collapse",
  props: {
    title: {}
  },
  emits: ["change"],
  setup(o, { emit: s }) {
    const e = o, t = s, n = v(!1), a = v(""), i = v(), r = () => {
      n.value = !n.value, a.value = `calc(${i.value.offsetHeight}px + 2rem)`, t("change", n.value);
    };
    return t("change", n.value), (u, m) => (c(), p("div", {
      class: S(["t-collapse", { open: n.value }]),
      style: T({ "--height": a.value })
    }, [
      $("div", {
        class: "t-collapse-header",
        onClick: r
      }, [
        h(u.$slots, "icon"),
        $("div", Rt, [
          h(u.$slots, "title", {}, () => [
            R(M(e.title), 1)
          ])
        ]),
        h(u.$slots, "toggle", {}, () => [
          n.value ? (c(), p("div", Dt, [...m[0] || (m[0] = [
            $("i", { class: "ri-arrow-up-s-line" }, null, -1)
          ])])) : (c(), p("div", Ot, [...m[1] || (m[1] = [
            $("i", { class: "ri-arrow-down-s-line" }, null, -1)
          ])]))
        ])
      ]),
      $("div", Nt, [
        $("div", {
          ref_key: "content",
          ref: i
        }, [
          h(u.$slots, "content", {}, () => [
            h(u.$slots, "default")
          ])
        ], 512)
      ])
    ], 6));
  }
}), Lt = /* @__PURE__ */ b({
  __name: "t-switch",
  props: {
    modelValue: { type: Boolean },
    color: { default: "primary" }
  },
  emits: ["update:modelValue"],
  setup(o, { emit: s }) {
    const e = o, t = s, n = () => {
      t("update:modelValue", !e.modelValue);
    }, a = x(() => {
      let i = {
        "--background": e.color,
        "--color": "#ffffff"
      };
      return ["warning", "info", "danger", "primary", "secondary", "success"].includes(e.color) && (i = {
        "--background": `var(--t-color-status-${e.color})`,
        "--color": `var(--t-color-status-${e.color}-text)`
      }), i;
    });
    return (i, r) => (c(), p("div", {
      class: S(["t-switch", { on: e.modelValue }]),
      style: T(a.value),
      onClick: n
    }, [...r[0] || (r[0] = [
      $("div", { class: "t-switch-icon" }, null, -1)
    ])], 6));
  }
}), Wt = /* @__PURE__ */ _(Lt, [["__scopeId", "data-v-3616970e"]]), Ht = /* @__PURE__ */ b({
  __name: "t-tab",
  props: {
    value: {}
  },
  setup(o) {
    const s = o, e = se("tabsState"), t = x(() => e.active.value === s.value), n = () => {
      e.setActive(s.value);
    };
    return (a, i) => (c(), p("li", {
      class: S(["t-tab", { active: t.value }])
    }, [
      W(j, {
        variant: "text",
        onClick: n,
        size: E(e).size,
        color: t.value && E(e).variant == "text" ? E(e).color : void 0
      }, {
        default: V(() => [
          h(a.$slots, "default", {}, void 0, !0)
        ]),
        _: 3
      }, 8, ["size", "color"])
    ], 2));
  }
}), Ut = /* @__PURE__ */ _(Ht, [["__scopeId", "data-v-36c12a50"]]), Xt = /* @__PURE__ */ b({
  __name: "t-tabs",
  props: {
    placement: { default: "top-start" },
    variant: { default: "border-under" },
    color: { default: "primary" },
    modelValue: {},
    border: { default: 30 },
    size: { default: "standard" }
  },
  emits: ["update:modelValue"],
  setup(o, { emit: s }) {
    const e = o, t = s, n = v("0px"), a = v(), i = x(() => {
      let u = e.color;
      return ["warning", "info", "danger", "primary", "secondary", "success"].includes(u) && (u = `var(--t-color-status-${u})`), u;
    }), r = () => {
      setTimeout(() => {
        if (e.placement.startsWith("top-") || e.placement.startsWith("bottom-")) {
          let u = a.value.querySelector(".active");
          if (u) {
            let m = u.getBoundingClientRect().left - a.value.getBoundingClientRect().left + a.value.scrollLeft, y = u.offsetWidth / 2;
            n.value = m + y - e.border / 2 + "px";
          }
        } else if (e.placement.startsWith("left-") || e.placement.startsWith("right-")) {
          let u = a.value.querySelector(".active");
          if (u) {
            let m = u.getBoundingClientRect().top - a.value.getBoundingClientRect().top + a.value.scrollTop, y = u.offsetHeight / 2;
            n.value = m + y - e.border / 2 + "px";
          }
        }
      }, 50);
    };
    return oe("tabsState", {
      active: x(() => e.modelValue),
      color: i.value,
      size: e.size,
      variant: e.variant,
      setActive: (u) => {
        t("update:modelValue", u), r();
      }
    }), K(() => {
      r();
    }), (u, m) => (c(), p("ul", {
      ref_key: "container",
      ref: a,
      class: S(["t-tabs", { [u.placement]: !0, [u.variant]: !0 }]),
      style: T({ "--color": i.value, "--transform": n.value, "--border": u.border + "px" })
    }, [
      h(u.$slots, "default", {}, void 0, !0)
    ], 6));
  }
}), jt = /* @__PURE__ */ _(Xt, [["__scopeId", "data-v-65a2c74c"]]), Ft = /* @__PURE__ */ b({
  __name: "t-loading-app",
  setup(o) {
    return (s, e) => (c(), B(ce, null, {
      default: V(() => [
        W(X, { type: "spinner" })
      ]),
      _: 1
    }));
  }
}), Gt = /* @__PURE__ */ _(Ft, [["__scopeId", "data-v-1436aa9f"]]), Kt = { key: 0 }, Jt = {
  key: 0,
  class: "t-checkbox-label"
}, Qt = /* @__PURE__ */ b({
  __name: "t-checkbox",
  props: {
    label: { default: "" },
    modelValue: { default: !1 },
    variant: {},
    type: { default: "check" },
    multiple: { type: Boolean, default: !1 },
    value: { default: void 0 },
    color: { default: "primary" }
  },
  emits: ["update:modelValue"],
  setup(o, { emit: s }) {
    const e = o, t = s, n = x(() => {
      let r = {
        "--background": e.color,
        "--color": "#ffffff"
      };
      return ["warning", "info", "danger", "primary", "secondary", "success"].includes(e.color) && (r = {
        "--background": `var(--t-color-status-${e.color})`,
        "--color": `var(--t-color-status-${e.color}-text)`
      }), r;
    }), a = x(() => e.multiple ? e.modelValue.includes(e.value) : e.value === void 0 ? e.modelValue : e.modelValue == e.value), i = () => {
      if (e.multiple) {
        let r = e.modelValue;
        e.modelValue.includes(e.value) ? r.splice(r.indexOf(e.value), 1) : r.push(e.value), t("update:modelValue", r);
      } else e.value === void 0 ? t("update:modelValue", !e.modelValue) : t("update:modelValue", e.value);
    };
    return (r, u) => (c(), p("div", {
      class: S(["t-checkbox", { active: a.value }]),
      style: T(n.value),
      onClick: i
    }, [
      $("div", {
        class: S(`t-checkbox-${r.type}`)
      }, [
        r.type == "radio" ? (c(), p("div", Kt)) : I("", !0),
        r.type == "check" ? h(r.$slots, "icon", { key: 1 }, () => [
          u[0] || (u[0] = $("i", { class: "ri-check-line" }, null, -1))
        ], !0) : I("", !0)
      ], 2),
      r.label ? (c(), p("span", Jt, M(e.label), 1)) : I("", !0)
    ], 6));
  }
}), Zt = /* @__PURE__ */ _(Qt, [["__scopeId", "data-v-d558d0b0"]]), Yt = /* @__PURE__ */ b({
  __name: "t-keyboard",
  setup(o) {
    const { height: s } = le();
    return (e, t) => (c(), p("div", {
      class: "t-keyboard",
      style: T({ "--height": E(s) + "px" })
    }, null, 4));
  }
}), eo = /* @__PURE__ */ _(Yt, [["__scopeId", "data-v-5d3fa1da"]]), to = /* @__PURE__ */ b({
  __name: "t-skeleton",
  props: {
    width: { default: "100%" },
    height: { default: "1rem" },
    radius: { default: "8px" },
    color: { default: null }
  },
  setup(o) {
    const s = o, e = x(() => ({
      "--width": s.width,
      "--height": s.height,
      "--radius": s.radius
    }));
    return (t, n) => (c(), p("div", {
      class: "t-skeleton",
      style: T(e.value)
    }, null, 4));
  }
}), oo = /* @__PURE__ */ _(to, [["__scopeId", "data-v-877bbdb5"]]), so = /* @__PURE__ */ b({
  __name: "t-ripple",
  props: {
    color: { default: "primary" }
  },
  setup(o) {
    const s = o, e = x(() => {
      let t = {};
      return s.color ? ["warning", "info", "danger", "primary", "secondary", "success"].includes(s.color) ? t = {
        "--color": "var(--t-color-status-" + s.color + ")"
      } : t = {
        "--color": s.color
      } : t = {
        "--color": "var(--t-color-surface)"
      }, t;
    });
    return (t, n) => (c(), p("div", {
      class: "t-ripple",
      style: T(e.value)
    }, null, 4));
  }
}), no = /* @__PURE__ */ _(so, [["__scopeId", "data-v-42cb3d71"]]), fo = () => ({
  install: (o) => {
    ye(), o.component("t-app", ke), o.component("t-screen", ie), o.component("t-swipe-screen", ot), o.component("t-cable", Xe), o.component("t-toolbar", nt), o.component("t-content", ce), o.component("t-card", je), o.component("t-refresher", Qe), o.component("t-button", j), o.component("t-back-button", at), o.component("t-present", D), o.component("t-text", F), o.component("t-sheet", ut), o.component("t-pull-signal", ue), o.component("t-input", vt), o.component("t-textarea", gt), o.component("t-rich-text", kt), o.component("t-grid", xt), o.component("t-grid-item", Bt), o.component("t-divider", Et), o.component("t-toggle-password", Mt), o.component("t-loading-icon", X), o.component("t-alert", re), o.component("t-avatar", At), o.component("t-collapse", qt), o.component("t-toast", ae), o.component("t-switch", Wt), o.component("t-tab", Ut), o.component("t-tabs", jt), o.component("t-loading-app", Gt), o.component("t-checkbox", Zt), o.component("t-keyboard", eo), o.component("t-skeleton", oo), o.component("t-ripple", no);
  }
});
export {
  _e as blurCurrentActive,
  io as createAction,
  co as createAlert,
  uo as createLoading,
  po as createToast,
  fo as createToife,
  ve as isFormElement,
  Re as presentController,
  w as screenController,
  le as useKeyboard
};
